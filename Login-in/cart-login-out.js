...
const selectedItem = (evt) => {
  ...
  if (typeof Storage !== undefined) {
    let newItem = {
      ...
      image: linkClicked.parentNode.querySelector('img').src, // Lưu đường dẫn ảnh
      quantity: 1,
    };
...
function addEventToAllCartButtons() {
  ...
  shoppingCart.addEventListener("click", function () {
    location.href = "showcart-login-in.html";
  });
...
function showCart() {
  if (localStorage.cartItems == undefined) {
    alert("Giỏ hàng của bạn trống. Vui lòng quay về trang chủ");
    location.href = "index-in.html";
  } else {
    let custommerCart = JSON.parse(localStorage.getItem("cartItems"));
    ...
    headColumns +=
      "<tr><th>No</th><th>Image</th><th>Product</th>" +
    ...
    footColumns +=
      ...
      "</td></tr>";
    footColumns +=
      '<tr><td colspan="4">Grand total:</td><td>' +
      formatCurrency(Math.floor(1.1 * total)) +
      "</td></tr>";
...
function deleteCart(elmt) {
  ...
  custommerCart.forEach((item) => {
    if (
      item.name !== elmt.parentElement.parentElement.children[2].textContent
    ) {
...
placeOrderBtn.addEventListener('click', () => {
  ...
  // Xóa hết item trong giỏ hàng (giả sử cartItems là mảng chứa danh sách sản phẩm)
  let cartItems = [];
  // Cập nhật giao diện
